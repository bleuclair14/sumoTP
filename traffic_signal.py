...
class TrafficSignal:
    ...
    def _pressure_reward(self):
        return -self.get_pressure()+self.get_teleport_punish()
    
    def _average_speed_reward(self):
        return self.get_average_speed()+self.get_teleport_punish()

    def _queue_reward(self):
        return -self.get_total_queued()+self.get_teleport_punish()

    def _diff_waiting_time_reward(self):
        ts_wait = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
        reward = self.last_measure - ts_wait
        self.last_measure = ts_wait
        return reward+self.get_teleport_punish()
    ...
    def get_teleport_punish(self):
        return -traci.simulation.getEndingTeleportNumber()*10
